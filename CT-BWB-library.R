...
## calculate test statistic in BS
GCT.tstat = function(X, khat, lrv.ba, type=1){
  ...
  if(type ==1 ){    Gn = max(apply(aa^2, 2, max));} # max2
  if(type ==2 ){    Gn = max(apply(aa, 2, max));} # max
  if(type == 3){    Gn = max(apply(aa^2, 2, sum));} ## sum 
...
GCT.break = function(X, c=1, wd, B=299, bsize, hsize, spatial = spatial, do.parallel, n.cl, type=1){
  ...
  GCT.novote2 = function(X, khat, B, bsize, hsize, do.parallel, n.cl, type){
    ...
    Tstat.WB = foreach(b=1:B, .combine='c', .packages=c('hdbinseg'), .export=c("LRV", "longrun_bartlett", "Demean", "GCT.tstat") ) %dopar% {
...
CTBWB.binary <- function(X, type, do.parallel=TRUE, n.cl=10, maxK=3, spatial=TRUE, critTF=TRUE){
      ...
      while (st > 0 && nmax < maxK) {
        ...
        for (j in brindex) {
          if (spindex[j]) {
            ...
            dat = X[id,];
            if(nrow(dat) > 20 ){ # set the minimum segment length as 20
  ...
  while(st > 0 && nmax < maxK){
    ...
    if(type == 1){ result$type = "BS-max2";}
    if(type == 2){ result$type = "BS-max";}
    if(type == 3){ result$type = "BS-sum";}
    if(type == 4){ result$type = "BS-DC";}
...
GCT.mosum = function(X, khat, lrv.ba){
  ...
  Gn1 = max(bb2); ## ms-max2
  Gn2 = max(bb); ## ms-max
#  Gn3 = max(apply(aa^2, 2, sum)); ## 
  Gn4 = sum(bb2); ## ms-sum
  return(c(Gn1, Gn2, Gn3=0, Gn4))
}

CTBWB.mosum = function(X, G, B, bsize, dwd, n.cl=10, spatial=TRUE, do.parallel=TRUE){
  ...
  ret$critical = aq[-3];
  ret$G = G;
  ret$ms.max2 = c(0, time.seq[br1], n);
  ret$ms.max = c(0, time.seq[br2], n);
  ret$ms.sum = c(0, time.seq[br4], n);
  return(ret)
}
...
##############################
# Error model E2 - ARMA(2,1)
##############################

ARMA21 = function(n, p, rho){
